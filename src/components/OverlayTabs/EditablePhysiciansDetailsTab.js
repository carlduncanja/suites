...
const EditablePhysiciansDetailsTab = ({ fields, onFieldChange }) => {
  ...
  const handleEmails = () => {
    ...
    const primaryEmail = fields.emails.filter(
  ...
  const handleEmergency = (value, key, contactIndex) => {
    const objIndex = emergencyContacts.findIndex(
    ...
    if (key === "name") {
      ...
      updatedObj = {
        ...emergencyContacts[objIndex],
        phone: formatNumber(formattedNumber),
      };
      ...
      <View style={{}}>
        {addresses.map((addressObj, index) => (
          <View key={index} style={{ flexDirection: "row", flex: 1 }}>
            <View
              style={[{ paddingRight: 35, width: "33%", marginBottom: 30 }]}
            >
              <View style={{ marginBottom: 5 }}>
              ...
              <View style={styles.inputWrapper}>
                <InputField2
                  onChangeText={(value) => {
                    ...
                    if (isValid) updatedAddress(value, "line1", addressObj._id);
                  }}
            ...
            <View style={styles.fieldWrapper}>
              ...
              <View style={styles.inputWrapper}>
                <InputField2
                  onChangeText={(value) => {
                    const isValid = validateAddressField(value);
                    if (isValid) updatedAddress(value, "line2", addressObj._id);
                  }}
    ...
    <>
      <View style={{}}>
        {emergencyContacts.map((contact, index) => {
          const isOpen = popoverList.filter(
            (item) => item.name === `emergency${index + 1}`
          );
          return (
            <View
              style={{ width: "100%", zIndex: index === 1 ? -1 : 0 }}
              key={index}
            >
                ...
                <View style={[styles.row]}>
                  ...
                  >
                    ...
                    <View style={styles.inputWrapper}>
                      <InputField2
                        onChangeText={(value) => {
                          const isValid = validateNameField(value);
                          if (isValid) handleEmergency(value, "name", index);
                        }}
                        value={contact.name}
                  ...
                  >
                    ...
                    <View style={styles.inputWrapper}>
                      <InputField2
                        onChangeText={(value) =>
                          handleEmergency(value, "relation", index)
                        }
                ...
                <View style={[styles.row]}>
                  ...
                  >
                    ...
                    <View style={styles.inputWrapper}>
                      <InputField2
                        onChangeText={(value) =>
                          handleEmergency(value, "phone", index)
                        }
                        value={contact.phone}
                  ...
                  >
                    ...
                    <View style={styles.inputWrapper}>
                      <InputField2
                        onChangeText={(value) =>
                          handleEmergency(value, "email", index)
                        }
                        value={contact.email}
